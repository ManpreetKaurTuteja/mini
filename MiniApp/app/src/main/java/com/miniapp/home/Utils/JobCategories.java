...
public class JobCategories {
    ...
    public JobCategories(MainActivity pActivity) {
        mainActivity = pActivity;
        miniApp = mainActivity.miniApp;
    }

    public void fetchCategories(final Fragment fragment) {
        if (miniApp.isConnected()) {
            ...
            String url = com.miniapp.commonUtils.Constants.ServiceConstants.SERVER_URL +
                    com.miniapp.commonUtils.Constants.ServiceConstants.GET_JOB_CATEGORIES_URL_EXTRA;
                ...
                public void onResponse(NetworkResponse response) {
                    ...
                    if (jobCategoriesResponse.getStatus() == com.miniapp.commonUtils.Constants.ResponseStatus.SUCCESS) {
                        if (fragment instanceof AddJobFragment) {
                            ((AddJobFragment) fragment).addSpineerData(jobCategoriesResponse.getJobsCategoryList());
                        } else {
                            ((MeFragment) fragment).addSpineerData(jobCategoriesResponse.getJobsCategoryList());
//                            JobCategoriesDailog categoriesDailog = new JobCategoriesDailog(mainActivity);
//                            if (!categoriesDailog.isShowingDialog()) {
//                                categoriesDailog.show(jobCategoriesResponse.getJobsCategoryList(), fragment);
//                            }
                        }
